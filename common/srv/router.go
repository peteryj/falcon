...
// Split a string in twain on sep.
// Doing it this way over strings.Split*() saves allocating a slice.
...
// Given a method and path, return the handler and vars that should be
// used to serve them.
func (r *router) route(
	method, path string, policy int) (http.Handler, map[string]string) {
	methodFound := false
	path = path[1:]
	slashCount := strings.Count(path, "/")
	// This code is slightly gnarly because it avoids allocating anything
	// until it's sure of a match.
NEXTMATCH:
	for _, m := range r.matchers {
		...
		methodFound = true
		if !m.hasCatchall && len(path) != 0 &&
			m.endSlash != (path[len(path)-1] == '/') {
			continue
		}
...
// HandlePolicy registers a handler for the given method, pattern, and
// policy header. The pattern is pretty much what you're used to,
// i.e. /static/:variable/*catchall
func (r *router) HandlePolicy(
	method, pattern string, policy int, handler http.Handler) {
...
// The pattern is pretty much what you're used to,
// i.e. /static/:variable/*catchall
...
func (r *router) Replicate(path string, handler http.Handler) {
	r.Handle(common.REPLICATE, path, handler)
}

func (r *router) Sync(path string, handler http.Handler) {
	r.Handle(common.SYNC, path, handler)
}
...
func (r *router) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	policy, err := strconv.Atoi(req.Header.Get(common.XBackendPolicyIndex))
	if err != nil {
		policy = 0
	}
	handler, vars := r.route(req.Method, req.URL.Path, policy)
	req = SetVars(req, vars)
	handler.ServeHTTP(w, req)
}

func routerNotFound(w http.ResponseWriter, req *http.Request) {
	pm, err := common.ParseProxyPath(req.URL.Path)
	if err == nil && pm["account"] == "" {
		common.CustomResponse(w, http.StatusPreconditionFailed, "bad url")
	} else {
...
func methodNotAllowed(w http.ResponseWriter, req *http.Request) {
	common.CustomResponse(w, http.StatusMethodNotAllowed, "Method Not Allowed")
}
