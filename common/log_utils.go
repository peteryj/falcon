...
func init() {
	var sink io.Writer
	syslog, err := gsyslog.NewLogger(gsyslog.LOG_NOTICE, "LOCAL0", "falcon")
	if err != nil {
...
func GetLogger(conf, name string) (*zap.Logger, error) {
	if conf == "" {
		BootstrapLogger.Println(
			"no config file path provided, return default logger which logs to /tmp/falcon.log and stderr")
		c := zap.NewProductionConfig()
		c.InitialFields = map[string]interface{}{"name": name}
		c.OutputPaths = append(c.OutputPaths, "/tmp/falcon.log")
		return c.Build()
	}
