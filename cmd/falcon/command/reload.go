...
func (c *ReloadCommand) Help() string {
	helpText := `
usage: falcon reload [server name]

  reload a server gracefully
`
...
func (c *ReloadCommand) Run(args []string) int {
	...
	if err := startServer(args[0]); err != nil {
		c.Logger.Printf("unable to reload server: %v", err)
		return EXIT_RESTART
	}

	c.Logger.Printf("%s server reloaded", args[0])
	return EXIT_OK
}

func (c *ReloadCommand) Synopsis() string {
	return "reload a server"
}
