...
import (
	...
	"github.com/iqiyi/falcon/objectserver/engine/pack"
)
...
func (c *PackReplicatorCommand) Help() string {
	helpText := `
Usage: falcon pack-replicator [-c config] [-once]

  Start replicator of pack engine
`
...
func (c *PackReplicatorCommand) Run(args []string) int {
	...
	if err := srv.RunDaemon(pack.InitReplicator, flags); err != nil {
		c.Logger.Printf("unable to run pack replicator: %v", err)
		return EXIT_START
	}
